...
import { hasSavedCosts, isOnboarded } from '@/lib/storage'
...
function LoginForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      setError('E-mail ou senha incorretos. A senha tem no mínimo 6 caracteres.')
      return
    }
  ...
  return (
    <div className="flex flex-col gap-[22px]">
      <div className="flex flex-col gap-2">
        <span className="label-uppercase text-[var(--color-brand-red)]">
          {expired ? 'Sessão expirada' : 'Acesse sua conta'}
        </span>
        <h1 className="text-display-md text-[var(--color-text)]">{expired ? 'Retome de onde parou' : 'Entrar'}</h1>
      </div>
      ...
      <form className="flex flex-col gap-[14px]" onSubmit={handleSubmit}>
        <Input
          ...
          className="h-[46px]"
          error={error && error.startsWith('E-mail inválido') ? ' ' : undefined}
          id="login-email"
        />
        <Input
          type="password"
        ...
        <Link
          href="/recuperar"
          className="self-start text-xs font-600 text-[var(--color-brand-red)]"
        >
          Esqueci minha senha
        </Link>
      ...
      <p className="text-xs text-[var(--color-text-secondary)]">
        Não tem conta?{' '}
        <Link href="/cadastro" className="text-[var(--color-brand-red)] font-600 hover:brightness-110">
          Criar gratuitamente
        </Link>
      </p>
    </div>
